...
define ("DB_NAME","psmafiq"); // set database name
...
if (!isset($_SESSION['user_id']) && !isset($_SESSION['user_name']) ) 
{
	...
	$rs_ctime = mysql_query("select `ckey`,`ctime` from `user` where `id` ='$cookie_user_id'") or die(mysql_error());
...
/* We also do authentication check of the `ckey` stored in cookie matches that stored in database during login*/
	 ...
	 if( !empty($ckey) && is_numeric($_COOKIE['user_id']) && isUserID($_COOKIE['user_name']) && $_COOKIE['user_key'] == sha1($ckey)  ) {
		...
		/* query user level from database instead of storing in cookies */	
		  list($user_level) = mysql_fetch_row(mysql_query("select user_level from user where id='$_SESSION[user_id]'"));
